...
- C++ functions have 0+ parameters (inputs) and have 0 or 1 output
...
robot.Wait(1000ms);
robot.StopMotors();
int line_brightness = robot.GetLightValue(LightSensor::CENTER);
double clearance = robot.GetUltrasonicDistance();
#+END_SRC
