...
for(int i = 0; i < mult_1; i++) {
    for(int j = 0; j < mult_2; j++) {
        cout << i * j << " ";
...
*********** Converting Time *******************
double time = 315; // very important here
char current_units = 's';
char desired_units = 'h';

if(current_units == 's' && desired_units == 'm') {
    time /= 60;
} else if(current_units == 's' && desired_units == 'h') {
    time /= 3600;
} else if(current_units == 'm' && desired_units == 's') {
    time *= 60;
} else if(current_units == 'm' && desired_units == 'h') {
    time /= 60;
} else if(current_units == 'h' && desired_units == 's') {
    time *= 3600;
} else if(current_units == 'h' && desired_units == 'm') {
    time *= 60;
}
cout << "converted " << current_units << " to " << desired_units << endl;
cout << "result = " << time;
...
*********** 99 bottles *******************
int number_bottles = 4;

for(int i = number_bottles; i > 0;) {
    char s = 's';
    if(i <= 1) {
        s = '\0';
    }
